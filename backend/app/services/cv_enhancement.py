...
def generate_pdf(phase4_data: dict, filepath: str):
    """
    Generate enhanced CV PDF from phase4_data and save it to filepath.
    """
    ...
    story.append(
        Paragraph("Professional Summary", h2_style)
    )

    story.append(
        Paragraph(
            phase4_data["rewritten_sections"]["summary"],
            body_style,
        )
    )
    ...
    for bullet in phase4_data["rewritten_sections"]["experience"]:

        story.append(
            Paragraph(
                f"<strike>{bullet['original']}</strike>",
                strike_style,
            )
        )
    ...
    story.append(
        Paragraph("Recommended Certificates", h2_style)
    )

    cert_table_data = [
        ["#", "Certificate", "Provider", "Why it matters"]
    ]

    for cert in sorted(
        phase4_data["certificates"],
    ...
    cert_table = Table(
        cert_table_data,
        colWidths=[1 * cm, 5 * cm, 3.5 * cm, 7.5 * cm],
    )
    ...
    for gap in sorted(
        phase4_data["skill_gaps"],
        key=lambda g: importance_order.get(
            g["importance"],
            9,
        ),
    ):

        color = importance_color.get(
            gap["importance"],
            colors.black,
        )
